...
class ADASPipeline:
    ...
    def process(
        ...
        # ── 2. Fog density (LLD module 2) ────────────────────────────────────
        fog_data         = estimate_fog_density(frame)
        fog_density       = fog_data.get("fog_density", 0.0)
        fog_data.setdefault("visibility", self._visibility_from_fog_density(fog_density))
        fog_data.setdefault("recommended_speed", self._recommended_speed_from_fog_density(fog_density))
        self.cached_fog_data = fog_data

        # NEW: Temporal fog prediction
        ...
        # ── 5. Red Glow / Brake-light detection (LLD module 5) ───────────────
        ...
        # NEW: LiDAR-based distances already computed in sensor fusion
        ...
        # ── 9. Risk Score Engine (LLD module 9) ──────────────────────────────
        ...
        # ── 10. Sensor-fusion context dict (LLD module 7) ────────────────────
        context = {
        ...
        # ── 11. Rule Engine → Alerts (LLD module 8) ──────────────────────────
        alerts = _rule_engine(
            context             = context,
        ...
        if llm_response and llm_response != self.last_llm_spoken:
            ...
            if alert_part and fog_density > 50 and risk_score > 50:
                ...
                self.last_llm_spoken = llm_response
